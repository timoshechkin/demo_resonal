...
function Date_Convert_for_Mysql($date_in)//Входной формат MySql: дд.мм.гггг
	...
	/*
	$uvolen = "AND UVOLEN = ''";
	if ($_GET['filtr_uvolen']=="1") $uvolen = "1"; else if($_GET['filtr_uvolen']=="") $uvolen = "AND UVOLEN = ''";
	
	$dekret = "AND DEKRET = ''";
	if ($_GET['filtr_dekret']=="1") $dekret = ""; else if($_GET['filtr_dekret']=="") $dekret = "AND DEKRET = ''";
	*/
	
	if ($_GET['filtr_uvolen']=="1" && $_GET['filtr_dekret']=="") $query_filtr = " AND UVOLEN='1' ";
	if ($_GET['filtr_uvolen']=="" && $_GET['filtr_dekret']=="1") $query_filtr = " AND DEKRET='1' ";
	if ($_GET['filtr_uvolen']=="1" && $_GET['filtr_dekret']=="1") $query_filtr = " AND (UVOLEN='1' OR DEKRET='1') ";
	if ($_GET['filtr_uvolen']=="" && $_GET['filtr_dekret']=="") $query_filtr = " AND UVOLEN='' AND DEKRET='' ";
	
	if ($_GET['listIdChild']!="") 
	{
		$queryID_podrazd = "ID_PODRAZD IN (".$_GET['idSel'].",".$_GET['listIdChild'].")";
	} 
	else 
	{
		$queryID_podrazd = "ID_PODRAZD = '".$_GET['idSel']."'";
	}
?>
<rows>
<?php
	...
	$query = mysqli_query($db,"SELECT * FROM PERSONAL WHERE ".$queryID_podrazd." AND DATE_BEG <='".Date_Convert_for_Mysql($_GET['date'])."' AND DATE_END >='".Date_Convert_for_Mysql($_GET['date'])."' ".$query_filtr." ORDER BY FAM ASC ");
		while ($list = mysqli_fetch_assoc($query))
			...
			if(isset($_GET["tabn"]) && $list["TABN"]==$_GET["tabn"]) $sel="selected='true'"; else $sel="";
			
			print("<row id='".$list["ID"]."' ".$sel." style='font-family:".$list_font['GRID_FONT_FAMILY']."; font-size:".$list_font['GRID_FONT_SIZE']."px;' >");
				print("<cell>");
                    print($list["TABN"]);
                print("</cell>");
				print("<cell>");
